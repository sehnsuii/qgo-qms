...
const Welcome = ({ auth, laravelVersion, phpVersion }) => {
            ...
            <html>
                ...
                <body>
                    <div class="ticket">
                        ...
                        <div class="info">Service: ${formData.appointmentType}</div>
    ...
    const handleSubmit = async (e) => {
        ...
        // Validate form data
        if (!formData.customerType || !formData.appointmentType) {
        ...
        try {
            ...
            const response = await fetch('/api/queuing', {
                method: 'POST',
                headers: {
            ...
            Swal.fire({
                ...
                html: `Your queue number is: <b>${data.queue_no}</b>`,
                icon: 'success',
                confirmButtonText: 'OK',
                willClose: () => {
                    printQueueTicket(data.queue_no);
                }
            });
            
            // Optionally reset form or redirect
            setFormData({ customerType: null, appointmentType: null });
            setStep(1);
                ...
                <div className="relative flex flex-1 w-full items-center justify-center">
                    <div className="bg-white bg-opacity-90 p-10 rounded-3xl shadow-lg w-[800px] min-h-[510px] flex flex-col items-center justify-center">
                        ...
                        {step === 1 && (
                            ...
                            //     <h2 className="text-4xl font-bold text-gray-800 mb-6">Welcome to Queuing System</h2>
                            //     <p className="text-xl text-gray-600 mb-10">Please follow the steps to get your queue number</p>
                            //     <img 
                            ...
                            //     <button
                            //         onClick={nextStep}
                            //         className="mt-6 px-10 py-4 bg-green-600 text-white text-lg font-bold rounded-lg shadow-md hover:bg-green-700 transition-colors"
                            //     >
                            //         Get Started
                            //     </button>
                            // </div>
                                ...
                                <div className="flex justify-center gap-8 mb-10">
                                    <button
                                        ...
                                        className={`px-8 py-12 rounded-xl shadow-md text-xl font-bold transition-all duration-200
                                            ${formData.customerType === 'priority' 
                        ...
                        {step === 3 && (
                            <div className="w-full text-center">
                                <h2 className="text-3xl font-bold text-gray-800 mb-6">Select Appointment Type</h2>
                                <p className="text-lg text-gray-600 mb-8">What type of service do you need?</p>
                                
                                <div className="grid grid-cols-2 gap-6 mb-10">
                                    {['Burial Assistance', 'Educational Assistance', 'Medical Assistance', 'Receiving', 'Schedule an appointment'].map((type) => (
                                        <button
                                            key={type}
                                            onClick={() => handleSelect('appointmentType', type)}
