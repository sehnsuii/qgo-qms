const CounterListItem = ({ counter }) => {
  ...
  let counterStateColor, borderColor, bgColor, gradientClass;
    ...
    <div className={`flex flex-col rounded-lg border-2 ${borderColor} ${bgColor} p-6 shadow-lg`}>
      ...
      <div className='flex flex-grow flex-col items-center justify-center gap-2'>
        {counter.status === 'offline' ? (
          <div className='text-center text-2xl font-medium text-gray-500'>Offline</div>
        ) : counter.status === 'break' ? (
          <div className='text-center text-2xl font-medium text-amber-500'>On Break</div>
        ) : servingQueue ? (
          <>
            <div className='text-center text-2xl font-medium text-red-800'>Now Serving</div>
          ...
          <div className='text-center text-2xl font-medium text-green-700'>Available</div>
        )}
      </div>
    </div>
  );
