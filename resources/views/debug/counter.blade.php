  ...
  @else
    ...
    <div class="flex flex-col">
      <div class="flex flex-row gap-2">
        <form method="POST" action="{{ route('counter.wait.debug', $counter) }}">
          @csrf
          @method('PATCH')
        ...
        <form method="POST" action="{{ route('counter.complete.debug', $counter) }}">
          @csrf
          @method('PATCH')
        ...
        <form method="POST" action="{{ route('counter.cancel.debug', $counter) }}">
          @csrf
          @method('PATCH')
